...
describe('Reset Password', () => {
    ...
    it('should send email with token and new password and return status 200', async () => {
        ...
        const searchUser = await UserModel.findOne({ email: userResetPassword.email }).select('+passwordResetToken passwordResetExpires');

        if (searchUser && token === searchUser.passwordResetToken && new Date() < user.passwordResetExpires) {
            searchUser.password = await passEncryption(userResetPassword.password)
        }
        await searchUser.save()

        const response = await request(app).post('/signin/reset_password').send(userResetPassword)
